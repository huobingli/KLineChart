...
import CustomCandlePane from './CustomCandlePane'
import CustomXAxisPane from './CustomXAxisPane'
...
import CustomSeparatorPane from './CustomSeparatorPane'

import ChartEvent from '../event/ChartEvent'
import { getPixelRatio } from '../utils/canvas'
import { throttle } from '../utils/performance'
import { createElement } from '../utils/element'
import CustomAnnotation from '../CustomComponent/overlay/CustomAnnotation'
import CustomTag from '../CustomComponent/overlay/CustomTag'
import { perfectOverlayFunc } from '../CustomComponent/overlay/CustomOverlay'
...
export default class CustomChartPane {
  constructor (container, styleOptions) {
    ...
    this._xAxisPane = new CustomXAxisPane({ id: XAXIS_PANE_ID, container: this._chartContainer, chartStore: this._chartStore })
    this._panes = new Map([[CANDLE_PANE_ID, new CustomCandlePane({
  ...
  _measurePaneHeight () {
    ...
    this._panes.get(CANDLE_PANE_ID).setYCustomAxisHeight(candlePaneHeight)
  ...
  _measurePaneWidth () {
    ...
    this._panes.forEach((pane, paneId) => {
      ...
      pane.setYCustomAxisWidth(yCustomAxisWidth)
      pane.setOffsetLeft(mainOffsetLeft, yAxisOffsetLeft)
      pane.setOffYCustomsetLeft(paneWidth - yCustomAxisWidth)
  ...
  createTechnicalIndicator (tech, isStack, options = {}) {
    ...
    this._separators.set(id, new CustomSeparatorPane(
      this._chartContainer,
      this._chartStore,
    ...
    const pane = new CustomTechnicalIndicatorPane({
      container: this._chartContainer,
      chartStore: this._chartStore,
      xAxis: this._xAxisPane.xAxis(),
      id,
   ...
   * @param paneId
   */
  createShape (CustomShapeTemplateClass, shapeOptions, paneId) {
    const {
    ...
    if (!this._chartStore.shapeStore().hasInstance(shapeId)) {
      ...
      const shapeInstance = new CustomShapeTemplateClass({
        id: shapeId,
        chartStore: this._chartStore,
        xAxis: this._xAxisPane.xAxis(),
        yAxis,
  ...
  createAnnotation (annotations, paneId) {
    ...
    annotations.forEach(({
      ...
      if (point && point.timestamp) {
        const annotationInstance = new CustomAnnotation({
